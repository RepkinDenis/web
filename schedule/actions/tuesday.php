...
if($user_id==999){
    ...
    VALUES ('$start', '$end', 'групповое занятие', '$teacher','Группа1', 'tuesday', '$user_id')");
}else if($user_id==1001){
    mysqli_query($connect, "INSERT INTO Schedule (`start`, `end`, type_lesson, teacher, student_name, day_of_week,  student_id) 
    VALUES ('$start', '$end', 'парное занятие', '$teacher','Пара1', 'tuesday', '$user_id')");
}else{
    mysqli_query($connect, "INSERT INTO Schedule (`start`, `end`, type_lesson, teacher, student_name, day_of_week,  student_id) 
    VALUES ('$start', '$end', '$type_lesson', '$teacher','$name_user', 'tuesday', '$user_id')");
}
